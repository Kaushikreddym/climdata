...
# ---------------------------------------------------------------------------
# 1. Discover sfc<DATE> sub-directories under svh20230401
# ---------------------------------------------------------------------------
log "Scanning ${CAST_PACKAGE} for sfc<DATE> sub-directories ..."
SFC_DIRS=()
while IFS= read -r entry; do
    [[ "$entry" =~ ^sfc[0-9]{8}$ ]] && SFC_DIRS+=("$entry")
done < <(list_children "${CATALOG_BASE}/${CAST_PACKAGE}" | sort -u)

[[ ${#SFC_DIRS[@]} -gt 0 ]] || die "No sfc<DATE> directories found under ${CAST_PACKAGE}. Check connectivity or that ${CAST_PACKAGE} exists."
log "Found ${#SFC_DIRS[@]} sfc dirs: ${SFC_DIRS[*]}"

# ---------------------------------------------------------------------------
# 2. Loop over each sfc<DATE> -> discover ensembles -> discover versions per var
# ---------------------------------------------------------------------------
TOTAL_OK=0
TOTAL_FAIL=0
...
for SFC in "${SFC_DIRS[@]}"; do
    ...
    # 2a. Discover ensembles (rXiYpZ pattern)
    ENSEMBLES=()
    while IFS= read -r entry; do
        [[ "$entry" =~ ^r[0-9]+i[0-9]+p[0-9]+$ ]] && ENSEMBLES+=("$entry")
    done < <(list_children "${CATALOG_BASE}/${CAST_PACKAGE}/${SFC}" | sort -u)
    ...
    for ENS in "${ENSEMBLES[@]}"; do
        ENS_DIR_OUT="${OUTDIR}/${SFC}/${ENS}"
        mkdir -p "$ENS_DIR_OUT"

        for VAR in "${VARIABLES[@]}"; do
            VAR_BASE="${CATALOG_BASE}/${CAST_PACKAGE}/${SFC}/${ENS}/${RCM}/${RCM_VER}/${FREQ}/${VAR}"

            # 2b. Discover version directories (v<DATE>) for this variable
            VERSIONS=()
            while IFS= read -r entry; do
            ...
            # 2c. List the actual .nc files under that version
            FILES=()
